...
The portfolio features an Iron Man-inspired Arc Reactor with an AI assistant that activates when users hover over the profile image. It uses the Web Speech API to provide a fully interactive voice experience:

### Features

- **Voice Recognition**: The AI listens and responds to user questions and commands using the browser's built-in speech recognition.
- **Natural Language Processing**: Basic NLP to understand user queries and provide relevant responses.
- **Conversational Interface**: The AI maintains the conversation context and history.
- **Visual Indicators**: Subtle UI indicators show when the AI is speaking or listening.
- **Iron Man Inspired**: The assistant's responses and voice are styled after JARVIS/FRIDAY from the Iron Man films.

### Browser Compatibility

The speech recognition features require a modern browser that supports the Web Speech API. Best compatibility is with:
- Google Chrome
- Microsoft Edge
- Safari (newer versions)

For browsers without speech recognition support, the visual Arc Reactor effect will still work, but the voice features may be limited.

### Privacy Notes

The speech recognition happens entirely in the browser - no audio data is sent to any server. The conversation history is stored only in your local browser storage.
